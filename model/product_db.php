    ...
    $query = '
    	  ...
    	  FROM products p
    	     INNER JOIN categories c ON p.categoryID = c.categoryID
          	 INNER JOIN level l ON p.level = l.levelID
...
function get_products_by_subcategory($subcategory_id) {
	...
	INNER JOIN categories c ON p.categoryID = c.categoryID
	INNER JOIN level l ON p.level = l.levelID
	WHERE p.subcategoryID = :subcategory_id';
	try {
		$statement = $db->prepare($query);
...
function get_product_images($product_id) {
	global $db;
	$query = '
	SELECT p.productID, pics.productImageFileName
	FROM products p
	INNER JOIN productImages pics ON p.productID = pics.productID
	WHERE p.productID = :product_id';
	try {
		$statement = $db->prepare($query);
		$statement->bindValue(':product_id', $product_id);
